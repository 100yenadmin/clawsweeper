...
For PRs, include a dedicated `deepRegressionReview` pass in addition to the normal functional/security/proof review. Use the provided OpenClaw deep-regression risk packet and any GitNexus advisory packet when present, but treat the live PR diff, checkout, and GitHub metadata as authoritative. For high or critical risk PRs, ask whether the branch solves the real problem, duplicates an existing architecture, stacks code on a broken path, or risks hidden core/runtime/auth/session/security/provider/release regressions. Set `status: "blocked"` when the PR should not merge without redesign or maintainer decision, `status: "needs_attention"` for material but non-blocking deep-regression risk, `status: "cleared"` when the high-risk pass found no concern, and `status: "not_applicable"` for non-PR items or truly docs/tests-only cases. Record `graphContextFreshness` honestly as `fresh`, `stale`, `missing`, `unknown`, or `not_applicable`; never claim graph-backed safety from stale or missing context. If this pass finds a P0-P2 concern, also reflect it in `overallCorrectness`, `maintainerDecision`, and `reviewFindings` when concrete file/line feedback is available.
...
Always fill `deepRegressionReview`. This is the dedicated architecture and
hidden-regression section. For non-PR items or docs/tests-only PRs, use
`status: "not_applicable"` and an empty concerns array unless the discussion
itself raises a concrete deep-regression risk. For high-risk PRs, do not use
`cleared` unless you can explain the proof boundary and graph/context freshness.
